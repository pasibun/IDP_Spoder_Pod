...
public class test {	
	...
	public static void main(String[] args) {
		...
		Imgproc.HoughCircles(src_gray, circles, Imgproc.CV_HOUGH_GRADIENT, 1, src_gray.rows()/8,200,100,0,0);
		
		
		Highgui.imwrite("ballooncircle.png", thresh_callback(src,180));
	}
	...
	static Mat thresh_callback(Mat src_gray, int thresh){
		...
		/*for(MatOfPoint contour : contours) {
			...
			Imgproc.drawContours(drawing, contours, 0, color,1,8, new Mat(), 0, new Point());
			Core.ellipse(drawing, minEllipse.get(0), color, 2,8);
			
			//MatOfPoint2f rect_points[]; minRects.add(points(rect_points)); 
		}*/
		
		for(int i = 0; i < contours.size() && i < minEllipse.size(); i++){
			Scalar color = new Scalar(Math.random()*255, Math.random()*255, Math.random()*255);
		...
		//Highgui.imwrite("ballooncircle.png", drawing);
		return drawing;
	}
}
